...
class Asset(ABC):
    ...
    def check_if_BOND_instance(self):
        if isinstance(self, Bond):
            return True
        elif isinstance(self,Stock):
            return False
        else:
            raise TypeError(f"{self} is not an instance of Asset")
        
    def __gt__(self, other):
        if Asset.check_if_BOND_instance(self):
...
class Bond(Asset):
    ...
    def __str__(self):
        return f"{self.description} {self.price}$ {self.yieldamt}"
